...
impl Spritesheet {
    ...
    /// spritesheet. It contains the width, height, and x and y of the sprite.
    ///
    /// The index file will match a spritesheet that can be saved with [`Self::save_spritesheet`].
    pub fn save_index_simple(&self, file_name_prefix: &str, minify: bool) -> std::io::Result<()> {
        ...
        let index = self.get_index()
            .iter()
            .map(|(name, &SpriteDescription { x, y, width, height, .. } )| {
                (name, SimpleSpriteDescription { x, y, width, height })
            })
