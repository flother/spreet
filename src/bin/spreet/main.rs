...
fn main() {
    ...
    let res = if args.simple_index_file {
        spritesheet.save_index_simple(&file_prefix, args.minify_index_file)
    } else {
        spritesheet.save_index(&file_prefix, args.minify_index_file)
    };
    if let Err(e) = res {
